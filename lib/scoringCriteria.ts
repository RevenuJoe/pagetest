...
- A LEAN, light navigation: logo on the left, ideally only 3 to 4 links in the middle. A bulky nav with 7+ links is a negative. CRITICAL: read the "Number of links inside <nav>: N" line in GROUND TRUTH. If N is 4 or fewer, the nav is NOT bulky — do NOT say "nav is bulky" or "navigation is bulky" or "slim the nav". Also CRITICAL: if "Number of links inside <nav>" is 0 but "Number of links/buttons inside <header>" is greater than 0, the page DOES HAVE top-area navigation/CTAs — it's just using <header> directly instead of <nav>. This is common in modern sites and is NOT a structural problem. Do NOT say "the page lacks a navigation entirely", "no navigation present", or "page has no nav" in this case — treat the header buttons as the navigation.
...
- Social proof visible above the fold: logos of recognised customers, ratings ("4.9/5 on G2"), or proof numbers ("Trusted by 10,000+ teams"). Missing social proof is a meaningful negative — but BEFORE you flag it as missing, look at the above-the-fold screenshot CAREFULLY. Recognisable company logos (Google, Microsoft, AWS, Salesforce, hospital names, brand wordmarks etc.), star ratings, "Trusted by", "Used by", "Loved by", G2/Capterra/Trustpilot badges, customer counts, follower counts, press logos — ANY of these qualify as social proof. If you can see even ONE of these in the above-the-fold viewport (desktop or mobile), the page HAS social proof above the fold. Do NOT recommend adding it. Do NOT say "social proof appears below the fold" if logos are visible in the above-the-fold screenshot. When in doubt that an element is social proof, err on the side of treating it as present.
